...
class GaussianMixture(torch.nn.Module):
    def __init__(self, n_hidden, n_components, n_mel_channels, fixed_gaussian,
        ...
        if not fixed_gaussian:
            ...
            self.register_buffer('mean', mean.half())   ################### fp16
            self.register_buffer('log_var', log_var.half()) ################### fp16

    def generate_mean(self, n_dimensions, n_components, scale=3):
        means = torch.eye(n_dimensions).half() ################### fp16
    ...
    def generate_log_var(self, n_dimensions, n_components):
        log_var = torch.zeros(1, n_dimensions, n_components).half() ################### fp16
        return log_var

    def generate_prob(self):
        return torch.ones(1, 1).half() ################### fp16
...
class Attention(torch.nn.Module):
    def __init__(self, n_mel_channels=80, n_speaker_dim=128,
        ...
        self.score_mask_value = -float("inf")  ################## fp16  float ??
...
class AR_Step(torch.nn.Module):
    ...
    def forward(self, mel, text, mask, out_lens):
        ...
        dummy = torch.HalfTensor(1, mel.size(1), mel.size(2)).zero_()  ################### fp16
    ...
    def infer(self, residual, text):
        ...
        dummy = torch.cuda.HalfTensor(1, residual.size(1), residual.size(2)).zero_() ################# fp16
